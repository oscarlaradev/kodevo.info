...
export const projectSchema = z.object({
  ...
  technologies: z.string().optional() // Hacer la cadena inicial opcional
    .transform((val): string[] => val ? val.split(',').map(tech => tech.trim()).filter(tech => tech.length > 0) : [])
    .pipe(
      z.string().array() // Asegurar que es un array de strings
        .refine(arr => arr.length > 0, { message: "Por favor, introduce al menos una tecnología válida." }) // Reemplaza .min(1)
        .refine(arr => arr.every(tech => tech.length <= 50), { message: "Cada tecnología no puede exceder los 50 caracteres." })
    ),
